...
func Test_requestLogger(t *testing.T) {
	...
	logStringResult := logBuffer.String()

	if logStringResult != expectedLogString {
		t.Errorf("Request result: '%s' does not match expected result: '%s'", logStringResult, expectedLogString)
	}

	if rr.Code != expectedStatusCode {
		t.Errorf("Response code '%d' does not match expected code: '%d'", rr.Code, expectedStatusCode)
	}
}
